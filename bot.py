...
# Emmy mascot - 3 poses, uploaded to fal.ai on first use
# Emmy mascot - images stored as separate files
EMMY_IMAGE_FILES = {
    "neutral": "emmy_neutral.png",
    "waving": "emmy_waving.png",
    "tech": "emmy_tech.png",
}
EMMY_URLS = {}  # populated on first upload: neutral/waving/tech -> url
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("✨ Оживить Эмми", callback_data="emmy_video")],
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "emmy_video":
        state["mode"] = "emmy_prompt"
        await query.message.reply_text(
            "✨ Оживляем Эмми!\n\n"
            "Напиши что она должна сделать или сказать:\n\n"
            "Примеры:\n"
            "— Эмми улыбается и показывает на аппарат VECTUS\n"
            "— Эмми говорит: Запишись на процедуру прямо сейчас!\n"
            "— Эмми радостно машет рукой и подмигивает"
        )
        return
...
async def process_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE, user_id: int, text: str):
    ...
    if mode == "emmy_prompt":
        state["mode"] = None
        await update.effective_message.reply_text("Генерирую видео с Эмми... 3-6 минут ⏳")
        try:
            url = await generate_emmy_video(update.effective_message, text)
...
async def upload_emmy_image(session, pose: str) -> str:
    """Upload Emmy image to fal.ai storage if not already uploaded"""
    global EMMY_URLS
    if pose in EMMY_URLS:
        return EMMY_URLS[pose]
    auth = {"Authorization": f"Key {FAL_KEY}"}
    with open(EMMY_IMAGE_FILES[pose], "rb") as _f:
        img_bytes = _f.read()
    async with session.post(
        "https://rest.alpha.fal.ai/storage/upload/initiate",
        headers=auth,
        json={"content_type": "image/png", "file_name": f"emmy_{pose}.png"}
    ) as r:
        init = await r.json(content_type=None)
        upload_url = init.get("upload_url", "")
        file_url = init.get("file_url", "")
    if upload_url:
        async with session.put(upload_url, data=img_bytes, headers={"Content-Type": "image/png"}) as r:
            logger.info(f"Emmy {pose} upload: {r.status}")
    EMMY_URLS[pose] = file_url
    logger.info(f"Emmy {pose} URL: {file_url}")
    return file_url


async def generate_emmy_video(msg, user_prompt: str) -> str:
    """Animate Emmy mascot using Kling image-to-video"""
    import json as _json

    MODEL = "fal-ai/kling-video/v1.6/standard/image-to-video"
    auth = {"Authorization": f"Key {FAL_KEY}"}

    # Choose pose based on keywords
    prompt_lower = user_prompt.lower()
    if any(w in prompt_lower for w in ["привет", "маш", "руку", "wave", "hello", "запишись", "призыв"]):
        pose = "waving"
    elif any(w in prompt_lower for w in ["технолог", "аппарат", "процедур", "эксперт", "объясн"]):
        pose = "tech"
    else:
        pose = "neutral"

    async with aiohttp.ClientSession() as session:
        if not EMMY_URLS.get(pose):
            await msg.reply_text(f"Загружаю Эмми ({pose}) на сервер... ⬆️")
        image_url = await upload_emmy_image(session, pose)

    # Build English prompt for Kling
    prompt_input = (
        f"Animate this 3D cartoon character named Emmy. She is wearing a green lab coat. "
        f"Action: {user_prompt}. "
        f"Keep her appearance exactly the same. Smooth natural animation. No text overlays."
    )
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            ...
            if not resp.ok:
                raise Exception(f"Submit failed {resp.status}: {await resp.text()}")
            data = _json.loads(await resp.text())
            request_id = data.get("request_id")
            status_url = data.get("status_url", f"https://queue.fal.run/{MODEL}/requests/{request_id}/status")
            response_url = data.get("response_url", "")
            logger.info(f"Emmy job: {request_id}")

        for attempt in range(120):
            await asyncio.sleep(5)
            if attempt > 0 and attempt % 12 == 0:
                mins = (attempt * 5) // 60
                await msg.reply_text(f"Kling AI анимирует Эмми... {mins} мин ⏳")
            try:
                async with session.get(status_url, headers=auth) as poll:
                    raw = (await poll.text()).strip()
                    ...
                    status = result.get("status", "")
                    logger.info(f"Emmy status #{attempt}: {status}")
                    if status == "COMPLETED":
                        res_url = result.get("response_url") or response_url
                        async with session.get(res_url, headers=auth) as res:
                            final = _json.loads(await res.text())
                            url = (final.get("video") or {}).get("url", "")
                            if url:
                                return url
                            raise Exception(f"No URL: {str(final)[:200]}")
                    if status in ("FAILED", "ERROR"):
                        raise Exception("Kling не смог анимировать")
            except Exception as e:
                if any(x in str(e) for x in ["FAILED", "No URL", "не смог"]):
                    raise
                continue
